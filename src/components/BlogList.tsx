...
// 【全局分形缓存 - 已预计算】
// ==========================================
const MAX_POINTS = 1400; // 大幅降低以提升滚动流畅度
...
// 【轻量化 Canvas - 支持暂停】
// ==========================================
const QuantumFieldCanvas = memo(({ seedId, paused = false }: { seedId: string | number; paused?: boolean }) => {
...
// 【防抖 Hover 包装器】
// ==========================================
const MagneticWrapper = memo(({ children, className }: { children: React.ReactNode; className?: string }) => {
...
// 【ListItem - memo 化】
// ==========================================
const ListItem = memo(({ post, index }: { post: Post; index: number }) => {
  ...
  return (
    ...
    >
      <div className="pointer-events-none absolute inset-0 opacity-0 transition-opacity duration-500 group-hover:opacity-100 z-0"
...
const BlogList: React.FC<{ posts: Post[]; currentPage: number; totalPages: number; onPageChange: (p: number) => void }> = ({
  ...
  // 鼠标移动 - 仅更新全局坐标（CSS 变量由 ListItem 自身读取）
      ...
      >
        <div className="relative w-[320px] h-[220px] rounded-sm border border-cyan-500/40 bg-black/80 shadow-[0_0_30px_rgba(6,182,212,0.3)] overflow-hidden p-1">
          ...
          <div className="w-full h-full relative opacity-80">
            <QuantumFieldCanvas seedId={hoveredPost.id} paused={false} />
          </div>
        </div>
      </motion.div>
    </AnimatePresence>,
