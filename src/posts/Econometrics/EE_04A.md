...
这表明 OLS 可能低估了教育对生育率的负面影响（即 OLS 结果偏向于 0），或者 IV 估计的是特定人群（LATE）的效应。这也可能暗示了遗漏变量（如能力或家庭背景）导致 OLS 存在正向偏差（Ability bias usually implies upward bias in wage, here maybe different dynamics, but the point is the estimates differ）。