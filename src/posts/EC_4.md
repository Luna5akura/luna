...
$\mathcal  D = \{\hat{\mathbf x_k},\mathbf u_k\}^N_{k=$}$ : decision-exogenous parameter pairs
- $N$: number
...
> -  $\Theta^{\mathrm{OPT}}(\mathbf{x}, \mathbf{u}):=\left\{\boldsymbol{\theta} \in \mathbb{R}^{d} \mid \mathbf{x} \in \mathcal{X}^{\mathrm{OPT}}(\boldsymbol{\theta}, \mathbf{u}),\|\boldsymbol{\theta}\|_{2}=$\right\}$
...
- $: AOG: Real return under current decision - real return under best decision
...
$ \mathbf{I O}(\mathcal{D}): \underset{\boldsymbol{\theta} \in \boldsymbol{\Theta}}{\operatorname{min}} \dfrac{$}{N} \sum_{k \in[N]} \ell\left(\hat{\mathbf{x}}_{k}, \mathcal{X}^{\mathrm{OPT}}\left(\boldsymbol{\theta}, \mathbf{u}_{k}\right)\right) $
...
$$
\begin{aligned} 
\text{min } & \theta_{$} x_{$}+\theta_{2} x_{2} \\ 
\text { s.t. } & x_{$}+u x_{2} \geq u \\ 
& 0 \leq x_{$} \leq u \\ 
& 0 \leq x_{2} \leq 2
\end{aligned}
$$
...
- $\mathcal U = \{u\}$
  - $u > $$, real constant
- $\mathcal D = \{\hat{\mathbf x_k}, u\}^N_{k=$} $
...
> - $ \boldsymbol{\theta}_{u}:=\left($ / \sqrt{$+u^{2}}, u / \sqrt{$+u^{2}}\right) $.

[Skip the proof](#end-proof-of-lemma-$)

#### Proof of Lemma $

Let : $\delta_u := \arccos(\dfrac{$}{\sqrt{$+u^2}})$
- $\cos\delta_u = \dfrac{$}{\sqrt{$+u^2}}, \sin\delta_u = \dfrac{u}{\sqrt{$+u^2}}$ 
  - when $ \hat{\boldsymbol{\theta}}_{k} \in \boldsymbol{\Theta}_{$}:= $ $ \left\{(\cos \delta, \sin \delta) \mid \delta \in\left(0, \delta_{u}\right]\right\} $, $ \hat{\mathbf{x}}_{k}=\tilde{\mathbf{x}}\left(\hat{\boldsymbol{\theta}}_{k}, u\right)=(0,$) $ almost surely. (Notice the slope)
...
$\hat{\boldsymbol{\theta}} \in \boldsymbol{\Theta}=\boldsymbol{\Theta}_{$} \cup \boldsymbol{\Theta}_{2}$

Then:

##### ($5)

$ \hat{\mathbf{x}}_{k}=\left\{\begin{array}{l}(0,$), \text { w.p. } 2 \delta_{u} / \pi \\ (u, 0), \text { w.p. }\left(\pi-2 \delta_{u}\right) / \pi\end{array}\right. $

$N_$$ : the numbers of $(0,$)$ in $\mathcal D$

$N_$$ : the numbers of $(u,0)$ in $\mathcal D$
...
$ \overline{\boldsymbol{\theta}}_{N}:=\underset{\boldsymbol{\theta} \in \boldsymbol{\Theta}}{\operatorname{argmin}} \frac{N_{$}}{N} l_{$}(\boldsymbol{\theta})+\frac{N_{2}}{N} l_{2}(\boldsymbol{\theta}) $


##### ($7)
- $ l_{$}(\boldsymbol{\theta})=\left\{\begin{array}{ll}0, & \text { if } \boldsymbol{\theta} \in \boldsymbol{\Theta}_{$} \\ \theta_{2}-u \theta_{$}, & \text { if } \boldsymbol{\theta} \in \boldsymbol{\Theta}_{2}\end{array}\right. $

##### ($8)
- $ l_{2}(\boldsymbol{\theta})=\left\{\begin{array}{ll}u \theta_{$}-\theta_{2}, & \text { if } \boldsymbol{\theta} \in \boldsymbol{\Theta}_{$}, \\ 0, & \text { if } \boldsymbol{\theta} \in \boldsymbol{\Theta}_{2} .\end{array}\right. $

Now discuss:

- When $N_$ > 0, N_2 > 0$ The minimum is $0$, occurs at $\boldsymbol \theta = (\cos\theta_u,\sin\delta_u)$
- $N_2 = 0$ : The minimum is $0$, occurs at $\boldsymbol \theta\in\boldsymbol \Theta _$$
- $N_$ = 0$ : The minimum is $0$, occurs at $\boldsymbol \theta \in\boldsymbol \Theta _2$
...
$ \mathbb{P}\left(N_{$} N_{2}>0\right) \leq \mathbb{P}\left(\overline{\boldsymbol{\theta}}_{N}=\left(\cos \delta_{u}, \sin \delta_{u}\right)\right) \leq $ $

With the probability distribution in [($5)](#$5), and $\mathcal D$ is generated i.i.d. from $\mathbb P_\theta $:

##### (20)

$ \mathbb{P}\left(N_{$} N_{2}>0\right)=$-\left(\dfrac{2 \delta_{u}}{\pi}\right)^{N}-\left($-\dfrac{2 \delta_{u}}{\pi}\right)^{N} $

it converges to $$$ as $N$ goes to infinity

So $ \mathbb{P}\left(\overline{\boldsymbol{\theta}}_{N}=\left(\cos \delta_{u}, \sin \delta_{u}\right)\right) $ converges to $ as $N $ goes to infinity (Squeeze Theorem)
...
> Then:
> For any $ v \in \mathbb{R}_{+} $there exists some $ \bar{u}>$ $ such that: 
...
[Skip the proof](#end-proof-for-proposition-$)
...
$ \mathcal{X}^{\mathrm{OPT}}\left(\boldsymbol{\theta}_{u}, u\right)=\left\{\left.\left(\frac{u t}{\sqrt{u^{2}+$}}, $-\frac{t}{\sqrt{u^{2}+$}}\right) \right\rvert\, t \in\left[0, \sqrt{u^{2}+$}\right]\right\} $

Since:

$\boldsymbol \theta ^* = (\cos\dfrac{\pi}{4},\sin\dfrac{\pi}{4})$ : ground-truth
$\mathbf x^* = (0,$)$: true optimal solution
...
&=\int_{0}^{\sqrt{u^{2}+$}} \frac{\sqrt{2}}{2 \sqrt{u^{2}+$}}\left(\frac{u t}{\sqrt{u^{2}+$}} + $-\frac{t}{\sqrt{u^{2}+$}}\right) d t-\frac{\sqrt{2}}{2}\\ 
&=\frac{\sqrt{2}(u-$)}{4}
\end{aligned}
$$
...
$\bar{u}_{\mathrm{AOG}} = 2\sqrt 2v + $$, s.t.:
...
> In Example $, let:  
> $ \overline{\mathbf{x}}_{\mathrm{IO}}(u)=\tilde{\mathbf{x}}\left(\boldsymbol{\theta}_{u}, u\right) $. 
> for any $ v \in \mathbb{R}_{+} $there exists some $ \bar{u}_{P O G}>$ $ such that :
...
When $\hat{\boldsymbol{\theta}} \in \boldsymbol{\Theta}_{$}:=\left\{(\cos \delta, \sin \delta) \mid \delta \in\left(0, \delta_{u}\right]\right\}$, we have: 

$\hat{\mathbf{x}}_{k}=\tilde{\mathbf{x}}(\hat{\boldsymbol{\theta}}, u)=(0,$)$,  with $\tilde{f}(\hat{\boldsymbol{\theta}}, u)=\hat{\theta}_{2}$ almost surely; 

When $\hat{\boldsymbol{\theta}} \in \Theta_{2}:=\left\{(\cos \delta, \sin \delta) \mid \delta \in\left(\delta_{u}, \pi / 2\right\}\right.$, we have : 

$\hat{\mathbf{x}}_{k}=\tilde{\mathbf{x}}(\hat{\boldsymbol{\theta}}, u)=(u, 0)$ with $\tilde{f}(\hat{\boldsymbol{\theta}}, u)=u \hat{\theta}_{$}$ almost surely. 
...
&= \int_{0}^{\delta_{u}} \int_{0}^{\sqrt{u^{2}+$}} \frac{$}{\sqrt{u^{2}+$}}\left[\frac{u t}{\sqrt{u^{2}+$}} \cos \delta+\left($-\frac{t}{\sqrt{u^{2}+$}}\right) \sin \delta-\sin \delta\right] d t d \delta \\ 
& +\int_{\delta_{u}}^{\pi / 2} \int_{0}^{\sqrt{u^{2}+$}} \frac{$}{\sqrt{u^{2}+$}}\left[\frac{u t}{\sqrt{u^{2}+$}} \cos \delta+\left($-\frac{t}{\sqrt{u^{2}+$}}\right) \sin \delta-u \cos \delta\right] d t d \delta \\ 
&= \frac{$}{2} \int_{0}^{\delta_{u}}(u \cos \delta-\sin \delta) d \delta+\frac{$}{2} \int_{\delta_{u}}^{\pi / 2}(-u \cos \delta+\sin \delta) d \delta \\ 
&= \sqrt{$+u^{2}}-\frac{u+$}{2} . \\ 
&> \frac{u-$}{2} \text{ (notice: } \sqrt{$+u^2}>u\text{)}
\end{aligned}
$$

Therefore, for any $\in\mathbb R_+$, exists:

$\bar{u}_{\mathrm{POG}}=2 v+$$, such that : 

$\operatorname{POG}\left(\overline{\mathbf{x}}_{\mathrm{IO}}\right)>v$ , for any $u>\bar{u}_{\mathrm{POG}}$.

###### End proof for lemma 5

#### End proof for proposition $
...
#### (9)
- $\mathcal{C}(\overline{\boldsymbol{\theta}}, \alpha):=\left\{\boldsymbol{\theta} \in \mathbb{R}^{d} \mid\|\boldsymbol{\theta}\|_{2}=$, \boldsymbol{\theta}^{\top} \overline{\boldsymbol{\theta}} \geq \cos \alpha\right\}$
  - $\alpha\in(0,\pi)$
...
> $\mathbb{P}\left[\mathrm{AOG}\left(\mathbf{x}_{\mathrm{CIO}}\right)=0\right] \rightarrow $$ and $\mathbb{P}\left[\mathrm{POG}\left(\overline{\mathbf{x}}_{\mathrm{CIO}}\right)<\pi / 2 \sqrt{2}\right] \rightarrow $$ as $N \rightarrow \infty$.
...
$\begin{aligned} \mathbf{C P}\left(\overline{\boldsymbol{\theta}}, \mathcal{D}_{\text {val }}, \gamma\right): \underset{\alpha,\left\{\boldsymbol{\theta}_{k}\right\}_{k \in \mathcal{K}_{\text {val }}}}{\operatorname{min}} & \alpha \\ \text { s.t. } & \hat{\mathbf{x}}_{k} \in \mathcal{X}^{\mathrm{OPT}}\left(\boldsymbol{\theta}_{k}, \mathbf{u}_{k}\right), \forall k \in \mathcal{K}_{\text {val }} \\ & \sum_{k \in \mathcal{K}_{\text {val }}} \mathbb{$}\left[\boldsymbol{\theta}_{k} \in \mathcal{C}(\overline{\boldsymbol{\theta}}, \alpha)\right] \geq \gamma\left(N_{\text {val }}+$\right) \\ & \left\|\boldsymbol{\theta}_{k}\right\|_{2}=$, \forall k \in \mathcal{K}_{\text {val }} \\ & 0 \leq \alpha \leq \pi,\end{aligned}$
...
- $\gamma \in[0,$]$ is a DM-specefied confidence level
...
>Let: 
> $\mathcal{D}_{\mathrm{val}}$ be a dataset, $\gamma \in[0,$], \overline{\boldsymbol{\theta}} \in \mathbb{R}^{d}, \tau=\left\lceil\gamma\left(N_{\mathrm{val}}+$\right)\right\rceil$ , $\Gamma_{\tau}$ be an operator that returns the $\tau^{\text {th }}$ largest value in a set. 
...
> - $c_{k}:=\max _{\boldsymbol{\theta}_{k}}\left\{\boldsymbol{\theta}_{k}^{\top} \overline{\boldsymbol{\theta}} \mid \hat{\mathbf{x}}_{k} \in \mathcal{X}^{\mathrm{OPT}}\left(\boldsymbol{\theta}_{k}, \mathbf{u}_{k}\right),\left\|\boldsymbol{\theta}_{k}\right\|_{2} \leq $\right\}$.

[Skip the proof](#end-proof-of-theorem-$)
...
$c:=\cos \alpha $, with $-$\le c \le $$

Introduce:

$y_k\in\{0,$\}$ that :
- $\hat{\boldsymbol{\Theta}}_{k}$intersect learned uncertainty set ( $=$$)
- not intersect : ($=0$)
...
$\begin{aligned} \max_{c,\{\boldsymbol \theta _k\}_{k\in\mathcal K_{\text{val}}},\{y_k\}_{k\in\mathcal K_{\text{val}}}}& c \\ \text { s.t. } & \hat{\mathbf{x}}_{k} \in \mathcal{X}^{\mathrm{OPT}}\left(\boldsymbol{\theta}_{k}, \mathbf{u}_{k}\right), \quad \forall k \in \mathcal{K}_{\text {val }} \\ & \boldsymbol{\theta}_{k}^{\top} \overline{\boldsymbol{\theta}} \geq c+2\left(y_{k}-$\right), \quad \forall k \in \mathcal{K}_{\text {val }} \\ & \sum_{k \in \mathcal{K}_{\text {val }}} y_{k} \geq\left\lceil\gamma\left(N_{\text {val }}+$\right)\right\rceil \\ & \left\|\boldsymbol{\theta}_{k}\right\|_{2}=$, \quad \forall k \in \mathcal{K}_{\text {val }} \\ & -$ \leq c \leq $ \\ & y_{k} \in\{0,$\}, \quad \forall k \in \mathcal{K}_{\text {val }} .\end{aligned}$

Notice decision variables $\boldsymbol \theta _k$ only interact in secont constraint, we can rewrite:

##### (40)

$\begin{array}{ll}\operatorname{max} & c \\ \text { s.t. } & c \leq c_{k}-2\left(y_{k}-$\right), \quad \forall k \in \mathcal{K}_{\text {val }} \\ & \sum_{k \in \mathcal{K}_{\text {val }}} y_{k} \geq\left\lceil\gamma\left(N_{\text {val }}+$\right)\right\rceil \\ & -$ \leq c \leq $ \\ & y_{k} \in\{0,$\}, \quad \forall k \in \mathcal{K}_{\text {val }},\end{array}$
...
$\begin{aligned} c_{k}:=\underset{\boldsymbol{\theta}_{k}}{\max } & \boldsymbol{\theta}_{k}^{\top} \overline{\boldsymbol{\theta}} \\ \text { s.t. } & \hat{\mathbf{x}}_{k} \in \mathcal{X}^{\mathrm{OPT}}\left(\boldsymbol{\theta}_{k}, \mathbf{u}_{k}\right) \\ & \left\|\boldsymbol{\theta}_{k}\right\|_{2} \leq $\end{aligned}$

Notice: optimal solution for [(40)](#40) is:

- $y_k = $$, for all $k$ s.t. $c_{k} \geq \Gamma_{\tau}\left(\left\{c_{k}\right\}_{k \in \mathcal{K}_{\text {val }}}\right)$
- $y_k = 0$, otherwise
  - $\tau = \dfrac{\gamma (N_{\text{val}}+$)}{N_{\text{val}}}$
...
> Let: 
> $\mathcal{D}_{\text {val }}$ : a dataset that satisfies Assumption $, 
...
> - for any $\gamma \in\left[0, N_{\text {val }} /\left(N_{\text {val }}+$\right)\right]$
> #### ($2)
> $\left|\mathbb{P}\left(\hat{\boldsymbol{\Theta}} \cap \mathcal{C}\left(\overline{\boldsymbol{\theta}}, \alpha_{\gamma}\right) \neq \varnothing\right)-\gamma\right| \leq \sqrt{\frac{8 \log \left(N_{v a l}+$\right)+2 \log N_{v a l}}{N_{v a l}}}+\frac{2}{N_{v a l}}$
> - with probability at least $$-\dfrac $ N_{\text {val }}$
> for any $\gamma \in[0,$ ]$
...
$ \begin{aligned} A_{\overline{\boldsymbol{\theta}}}(\hat{\mathbf{x}}, \mathbf{u}):=\underset{\boldsymbol{\theta}}{\operatorname{max }} & \boldsymbol{\theta}^{\mathrm{\top}} \overline{\boldsymbol{\theta}} \\ \text { s.t.  } & \boldsymbol{\theta} \in \boldsymbol{\Theta}^{\mathrm{OPT}}(\hat{\mathbf{x}}, \mathbf{u}) \\ & \|\boldsymbol{\theta}\|_{2} \leq $ .\end{aligned} $

Notice:

$ c_{k}=A_{\overline{\boldsymbol{\theta}}}\left(\hat{\mathbf{x}}_{k}, \mathbf{u}_{k}\right) $ for any $ k \in \mathcal{K}_{\text {val }} $
- $c_k$ is defined in [Theorem $](#theorem-$)
...
$ \mathcal{H}=\left\{h(z, \alpha)=\mathbb{$}\left[\boldsymbol{\Theta}^{\mathrm{OPT}}(\hat{\mathbf{x}}, \mathbf{u}) \cap \mathcal{C}(\overline{\boldsymbol{\theta}}, \alpha)\right] \mid \alpha \in(0, \pi)\right\} $

##### (36)

$ \Pi_{\mathcal{H}}(N):=\max _{\left(Z_{$}, Z_{2}, \ldots, Z_{N}\right) \in \mathcal{Z}^{N}}\left|\left\{\left(h\left(Z_{$}\right), h\left(Z_{2}\right), \ldots, h\left(Z_{N}\right)\right) \mid h \in \mathcal{H}\right\}\right| $

We can see:

##### (45)

$ \Pi_{\mathcal{H}}\left(N_{\mathrm{val}}\right)=N_{\mathrm{val}}+$ $

##### Lemma 9

> Let $ \mathcal{H} $ be a class of functions taking values in $ \{$,-$\} $, 
> Then, for any integer $ N \geq $ $, the following holds
> $
...
$ \gamma \leq \frac{$}{N_{\text {val }}} \sum_{k \in \mathcal{K}_{\text {val }}} h\left(z_{k}, \alpha\right) \leq \gamma+\frac{2}{N_{\text {val }}} $.

##### (46)

$ \Re_{N_{\mathrm{val}}}(\mathcal{H}) \leq \sqrt{\dfrac{2 \log \left(N_{\mathrm{val}}+$\right)}{N_{\mathrm{val}}}} $

Notice, $\alpha$ is the smallest value that:

##### (47)

$ \dfrac{$}{N_{\text {val }}} \sum_{k \in \mathcal{K}_{\text {val }}} h\left(z_{k}, \alpha\right)=\dfrac{$}{N_{\text {val }}} \sum_{k \in \mathcal{K}_{\text {val }}} \mathbb{$}\left[\boldsymbol{\Theta}^{\mathrm{OPT}}\left(\hat{\mathbf{x}}_{k}, \mathbf{u}_{k}\right) \cap \mathcal{C}(\overline{\boldsymbol{\theta}}, \alpha)\right]=\dfrac{\left\lceil\gamma\left(N_{\text {val }}+$\right)\right\rceil}{N_{\text {val }}} $

So:

$ \gamma \leq \dfrac{$}{N_{\text {val }}} \sum_{k \in \mathcal{K}_{\text {val }}} h\left(z_{k}, \alpha\right) \leq \gamma+\dfrac{2}{N_{\text {val }}} $.

##### Lemma $0

> For any b-uniformly bounded class of functions $ \mathcal{F} $, any positive integer $ N \geq $ $, and any scalar $ \delta \geq 0 $, 
> with probability at least:  
> $ $-\exp \left(-N \delta^{2} /\left(2 b^{2}\right)\right) $, 
> we have : 
> $\sup _{f \in \mathcal{F}}\left|\frac{$}{N} \sum_{i \in[N]} f\left(X_{i}\right)-\mathbb{E}\left[f\left(X_{i}\right)\right]\right| \leq 2 \mathfrak{R}_{N}(\mathcal{F})+\delta$  
> - $ \mathfrak{R}(\mathcal{F}) $ denotes the Rademacher complexity of the function class $ \mathcal{F} $.

Then with probability at least $\delta = $ - \dfrac{$}{N_\text{val}}$ :

##### (50)

$ \left|\dfrac{$}{N_{\text {val }}} \sum_{k \in \mathcal{K}_{\text {val }}} h\left(z_{k}, \alpha\right)-\mathbb{E}[h(z, \alpha)]\right| \leq 2 \mathfrak{R}_{N_{\text {val }}}(\mathcal{H})+\sqrt{\dfrac{2 \log N_{\text {val }}}{N_{\text {val }}}} $
