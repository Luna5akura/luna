const t=`**第三章 基于资源满意度的门诊动态调度模型构建**  
（续）
...
**【解释与管理意义】**  
正态近似法将原本复杂的随机约束转化为确定性凸优化问题，大幅提升了计算效率，同时保持了对极端事件的控制能力。在扩展模型（Model 3）中，由于引入了动态分流比例 \\( p_{it} \\) 和诊室异构参数，模型自动实现了智能负载均衡与协同调速，最优 \\( \\rho \\) 通常显著低于单诊室情形。在实际应用中，管理者可根据历史数据验证正态假设的合理性；若方差较大，可进一步采用更精确的分布进行灵敏度分析。该方法是本文所有 RSI 模型求解的核心技术支撑，确保动态速率决策与分流策略既稳健又可操作。`;export{t as default};
