...
If the objective function $ f(\\boldsymbol{x}) $ is linear, then the following problems are equivalent:
(1) minimize $ f(\\boldsymbol{x}) $
subject to $ \\boldsymbol{x} \\in \\mathbb{R}^{n} $
$ \\boldsymbol{x} \\in S $ (feasible set)
(2) minimize $ f(\\boldsymbol{x}) $
$ \\Longleftrightarrow \\quad $ subject to $ \\boldsymbol{x} \\in \\mathbb{R}^{n} $
$ \\boldsymbol{x} \\in \\operatorname{co}(S) $ (convex hull of $ S $ )
