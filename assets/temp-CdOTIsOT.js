...
这种现象被称为“采样引起不可控性”。它发生在当采样频率与系统的自然频率之间存在特定关系时，导致输入在采样点上对系统的某个模态失去控制能力。对于本题中的系统，其特征值为 $ \\lambda = \\pm i $，对应的是一个振荡频率为 1 rad/s 的谐振环节，其周期为 $ 2\\pi $。当采样周期 T 是振荡周期的一半（$ \\pi $）的整数倍时，采样点恰好落在系统状态循环的特定位置，使得控制输入无法影响系统的状态，从而导致能控性丧失。`;export{n as default};
